...
AirsimROSWrapper::AirsimROSWrapper(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private, const std::string& host_ip)
    ...
    , is_used_lidar_timer_cb_queue_(true)   //dieptran    // false
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings) {
                ...
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    ...
                    image_pub_vec_.push_back(image_transporter.advertise(cam_image_topic, 1));  //1 - DuyNguyen
    ...
    if (publish_clock_) {
        clock_pub_ = nh_private_.advertise<rosgraph_msgs::Clock>("clock", 0.5);  // 1 - DuyNguyen
    }
...
sensor_msgs::PointCloud2 AirsimROSWrapper::get_lidar_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name) const
{
    ...
    for(uint8_t i=0; i< lidar_data.point_cloud.size(); i++)
    {
        list_intensity.push_back(intensity_0 * exp((-0.004)*(distance(lidar_data.pose.position.x(), lidar_data.pose.position.y(), lidar_data.pose.position.z()))));
    }
    ...
    if (lidar_data.point_cloud.size() > 3)
    {
        ...
        std::vector<float> data_std_2 = data_std;
        ...
        //vector<float> my_vector = {0};

        // for(uint i=0; i<data_std.size(); i++)
        // {
        //     if(i%3 == 0)
        //     {
        ...
        data_std.insert(data_std.begin()+3, a);

        // int count = 0;

        // for(uint8_t i=0; i<data_std.size(); i++)
        // {
        //     if(i%16 == 0)
        //     {
        //         for(int j=0; j<4; j++)
        //         {
        //             data_std.data().insert((data_std.begin()+i+j, list_intensity.at(count)));
        //             count++;
        //         }
        //     } 
        // }
        ...
        //         for(int j=0; j<12; j++)
        //         {
        //             data_std.data().insert((data_std.begin()+i+j, 0));
        //         }
        //     }
        // }

        //Kiểu unsigned char: lưu các số nguyên dương từ 0 đến 255.
        const unsigned char* bytes = reinterpret_cast<const unsigned char*>(data_std.data()); //ep kieu tu float ve unsigned char
        vector<unsigned char> lidar_msg_data(bytes, bytes + sizeof(float) * data_std.size()); //bytes la ptu dau tien  
                                                                                              //bytes + sizeof(float) * data_std.size() la pt cuoi cung
...
sensor_msgs::PointCloud2 AirsimROSWrapper::get_lidar_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name) const
{
    ...
    if (lidar_data.point_cloud.size() > 3) {
        ...
        std::vector<float> data_std = lidar_data.point_cloud;
        const unsigned char* bytes = reinterpret_cast<const unsigned char*>(data_std.data());  //ep kieu tu float ve unsigned char
        std::vector<unsigned char> lidar_msg_data(bytes, bytes + sizeof(float) * data_std.size());  //luu tru mang phan tu sang vector vua tao
